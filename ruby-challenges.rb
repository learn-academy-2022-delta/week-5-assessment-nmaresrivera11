...
def particular_lett array, letter
    array.select {|word| word.include?(letter)}
end
p particular_lett(beverages_array, letter_o)
p particular_lett(beverages_array, letter_t)
...
p sum_of_all(nums_array1)
p sum_of_all(nums_array2)
...
class Bike
    ...
    def pedal_faster speed
        @current_speed += speed
        # (+=) in Ruby is shorthand to add then be equal to
    end
    def brake_bike speed
...
p my_bike.brake_bike(5)
# Expected output example: my_bike.brake(5) => 23
p my_bike.brake_bike(25)
